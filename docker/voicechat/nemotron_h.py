...
class NemotronHModel(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self._has_asr = "asr_logits" in getattr(config, "custom_outputs", [])
...
class NemotronHForCausalLM(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        self._log(f"__init__: vllm_config = {vllm_config}")
        ...
        custom_outputs = getattr(config, "custom_outputs", [])
        self._has_asr = "asr_logits" in custom_outputs
        self.asr_head = (
            ParallelLMHead(
                config.vocab_size,
                config.hidden_size,
        ...
        self._has_fc = "function_logits" in custom_outputs
        self.function_head = (
            ParallelLMHead(
                config.vocab_size,
                config.hidden_size,
        ...
        for key, val in os.environ.items():
            if key.startswith("VLLM_ASR_BOOST_"):
                ...
                if boost != 0.0:
                    ...
                    self._log(f"__init__: asr_logit_boosts = {self.asr_logit_boosts}")
            elif key.startswith("VLLM_TEXT_BOOST_"):
                token_id = int(key[len("VLLM_TEXT_BOOST_"):])
                boost = float(val)
                if boost != 0.0:
                    self.text_logit_boosts[token_id] = boost
                    self._log(f"__init__: text_logit_boosts = {self.text_logit_boosts}")

        self.make_empty_intmd_tensors = self.model.make_empty_intmd_tensors

    _LOG_PATH = "/scripts/nemotron_h_debug.log"

    def _log(self, msg: str):
        try:
            with open(self._LOG_PATH, "a") as f:
                f.write(f"[NemotronH pid={os.getpid()}] {msg}\n")
        except Exception:
            pass
    ...
    def forward(
        ...
        **kwargs,
        ...
        results = [hidden_states, text_logits]

        if self._has_asr and self.asr_head is not None:
            ...
            # Sanitize NaN/Inf before argmax — copied lm_head can produce NaN logits
            # for GA FC checkpoint (no real ASR head), causing garbage token IDs ≥ vocab_size
            asr_logits = torch.nan_to_num(asr_logits, nan=0.0, posinf=1e4, neginf=-1e4)
            asr_tokens = torch.argmax(asr_logits, dim=1)
            asr_tokens = asr_tokens.clamp(0, self.unpadded_vocab_size - 1)
            results.extend([asr_tokens, asr_logits])
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Materialize weights to detect which keys are present in the checkpoint.
        weights_list = list(weights)
        weight_keys = {name for name, _ in weights_list}

        loader = AutoWeightsLoader(self)
        loaded = loader.load_weights(iter(weights_list), mapper=mapper)

        # If the checkpoint has no ASR weights but the model has asr_head/embed_asr_tokens
        # layers (because custom_outputs includes asr_logits), vLLM would raise
        # "not initialized from checkpoint". Mark them as loaded so the check passes;
        # they remain randomly initialized (outputs are unused in voicechat inference).
        if self._has_asr:
            asr_in_ckpt = any(
                "asr_head" in k or "embed_asr_tokens" in k for k in weight_keys
            )
            if not asr_in_ckpt:
                for name, _ in self.named_parameters():
                    if "asr_head" in name or "embed_asr_tokens" in name:
                        loaded.add(name)
