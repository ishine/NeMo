...
class TestStarcoder_15B:
    ...
    def recipe_module(self):
        return starcoder_15b

    def test_model(self, recipe_module):
        model_config = recipe_module.model()
        assert isinstance(model_config, run.Config)
        assert model_config.__fn_or_cls__ == StarcoderModel
        assert isinstance(model_config.config, run.Config)
        assert model_config.config.__fn_or_cls__ == StarcoderConfig15B

    def test_trainer(self, recipe_module):
        trainer_config = recipe_module.starcoder_trainer()
        ...
        assert trainer_config.strategy.ckpt_include_optimizer is True
    ...
    def test_pretrain_recipe(self, recipe_module):
        ...
        assert recipe.model.__fn_or_cls__ == StarcoderModel
