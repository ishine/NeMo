...
from nemo.collections.llm.recipes import qwen3_235b_a22b
from nemo.lightning import Trainer


class TestQwen3_235B_A22B:
    ...
    def test_pretrain_recipe(self, recipe_module):
        ...
        assert recipe.data.micro_batch_size == 1  # Reduced for very large model

        # Check strategy configuration - MoE specific with large parallelism
        assert recipe.trainer.strategy.tensor_model_parallel_size == 4
        assert recipe.trainer.strategy.pipeline_model_parallel_size == 16
        assert recipe.trainer.strategy.expert_model_parallel_size == 8
        assert recipe.trainer.strategy.expert_tensor_parallel_size == 1
        assert recipe.trainer.strategy.sequence_parallel == True
        assert recipe.trainer.strategy.context_parallel_size == 2
        assert recipe.trainer.num_nodes == 16
        ...
        assert recipe.data.tokenizer.pretrained_model_name == "Qwen/Qwen3-235B-A22B"

        # Check pipeline split configuration
        assert recipe.trainer.strategy.account_for_embedding_in_pipeline_split == True
        assert recipe.trainer.strategy.account_for_loss_in_pipeline_split == True
    ...
    def test_finetune_recipe(self, recipe_module):
        ...
        # Check MoE strategy configuration for finetuning
        assert recipe.trainer.strategy.tensor_model_parallel_size == 4
